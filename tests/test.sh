...
testGet() {
    rm -rf /data/*
    curl -s http://$URL/very/long/path?param=value
...
testPost() {
    rm -rf /data/*
    curl -s -X POST -d '{"param":"value"}' http://$URL
...
testPut() {
    rm -rf /data/*
    curl -s -X PUT -d '{"param":"value"}' http://$URL
